...
@section('content')
    ...
    <div class="container">
        <div class="row" id="genres">
            <ul>
                <li><a href="{{ route('cine.user.admin', ['user' => $userUrl]) }}" class="{{ isset($filterGenre) ? 'dynamic-link' : 'btn btn-primary' }}">Todo</a></li>
                @foreach($genres as $genre)
                    <li><a href="{{ route('cine.user.admin.genre', ['user' => $userUrl, 'id' => $genre->id]) }}" class="{{ isset($filterGenre) && $filterGenre == $genre->id ? 'btn btn-primary' : 'dynamic-link' }}">{{ $genre->name }}</a></li>
                @endforeach
            </ul>
        </div>
